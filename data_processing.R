...
combined_emoji <- rbind(pipi_emoji)
...
combined_count_messages <- rbind(pipi_count_messages)


#####common_strings#####
pipi_common_strings <- read_csv("data/pozor/0_query_most_common_strings.csv")
...
pipi_time_respond$day_send <- factor(pipi_time_respond1$day_send, levels = c("poniedziałek","wtorek", "środa", "czwartek","piątek","sobota","niedziela"))

combined_time_respond <-rbind(pipi_time_respond)

#####other####

# formatted_time <- "00:00"
# formatted_time1 <- "23:59"
# 
# time_start <- as.POSIXct(formatted_time, format = "%H:%M")
# time_end <- as.POSIXct(formatted_time1, format = "%H:%M")
# 
# time_sequence <- seq(time_start, time_end, by = "60 mins")
# time_sequence = format(time_sequence, "%H:%M")

time_sequence <- c("00:00", "01:00", "02:00", "03:00", "04:00", "05:00", "06:00", "07:00", "08:00",
...
days <- c("poniedziałek", "wtorek", "środa", "czwartek", "piątek", "sobota", "niedziela")
